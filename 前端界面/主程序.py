...
def call_deepseek(symbol, price, signals=None):
    if not DEEPSEEK_API_KEY:
        return None
    prompt = f"你是量化交易AI。分析品种: {symbol}, 价格: {price}, 策略信号: {signals}. 输出buy/sell/hold"
    try:
        resp = requests.post(
...
# ================== AI交易 ==================
    ...
    if st.button("启动AI交易", type="primary"):
        with st.spinner("AI分析中..."):
            ...
            final = ai_decision if ai_decision else "hold"
            
            st.success(f"AI最终决策: {final.upper()}")
            
            with st.expander("查看各策略信号"):
                for name, sig in signals.items():
                    st.write(f"{name}: {sig.upper()}")
...
st.markdown("---")
st.caption("量化交易系统 v5.0 | Powered by DeepSeek AI")
