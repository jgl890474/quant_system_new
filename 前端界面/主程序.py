...
<style>
    ...
    .detail-popup { 
        ...
        padding: 20px; border: 1px solid #00d2ff; 
        z-index: 1000; width: 520px; max-height: 80vh; overflow-y: auto; 
    }
    .detail-popup h4 { color: #00d2ff; margin-bottom: 12px; font-size: 14px; }
    .close-btn { 
        background-color: #00d2ff; color: #000; border: none; 
        padding: 6px 16px; border-radius: 16px; 
        cursor: pointer; margin-top: 15px; width: 100%; font-size: 12px; 
        font-weight: bold;
    }
...
def show_popup():
    if st.session_state.show_popup:
        ...
        with st.container():
            ...
            # 专属关闭按钮 样式强化
            if st.button("🔒 关闭弹窗", key="close_popup", use_container_width=True):
                st.session_state.show_popup = False
                st.rerun()
...
with col4:
    ...
    if st.button("📊 详情", key="monthly_btn", use_container_width=True):
        st.session_state.show_popup = True
        st.session_state.popup_type = "monthly_btn"
        st.rerun()
