...
<style>
    ...
    .detail-popup { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: #1e1e2e; border-radius: 12px; padding: 20px; border: 1px solid #00d2ff; z-index: 1000; width: 560px; max-height: 80vh; overflow-y: auto; }
    .detail-popup h4 { color: #00d2ff; margin-bottom: 12px; font-size: 14px; }
    .close-btn { background-color: #00d2ff; color: #000; border: none; padding: 6px 16px; border-radius: 16px; cursor: pointer; margin-top: 12px; width: 100%; font-size: 12px; font-weight: bold; }
    .close-btn:hover { background-color: #00ff88; color: #000; }
...
def get_daily_returns():
    ...
    returns = [120, -80, 250, 180, -40, 320, 210]
...
def get_strategy_pnl():
    return [("期货趋势", 2140), ("外汇利差", 1280), ("加密双均线", 3450), ("外汇突破", 980), ("期货均值", 450)]
    ...
    if st.session_state.show_popup and st.session_state.popup_type:
        ...
        with st.container():
            ...
            if st.session_state.popup_type == "asset":
                ...
                fig = go.Figure(data=go.Scatter(x=dates, y=values, mode='lines+markers', line=dict(color='#00d2ff', width=2), marker=dict(size=4, color='#00ff88')))
                fig.update_layout(height=280, paper_bgcolor="#1e1e2e", plot_bgcolor="#1a1d24", font_color="#e6e6e6", margin=dict(l=0, r=0, t=20, b=0), xaxis_title="日期", yaxis_title="资产(USD)")
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**各策略收益贡献**")
                for name, pnl in get_strategy_pnl():
                    st.write(f"{name}: +${pnl:,}")
                ...
                fig.update_layout(height=250, paper_bgcolor="#1e1e2e", plot_bgcolor="#1a1d24", font_color="#e6e6e6", margin=dict(l=0, r=0, t=20, b=0), xaxis_title="日期", yaxis_title="收益(USD)")
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**今日交易**")
                st.write("✅ 买入 EURUSD @ 1.0850")
                st.write("✅ 买入 BTC @ 45000")
                st.write("📊 AI信号: 买入 (置信度87%)")
                ...
                st.dataframe(df[['品种', '数量', '现价', '盈亏']], use_container_width=True, hide_index=True)
                # 饼图
                values = [abs(h['盈亏']) for h in holdings]
                labels = [h['品种'] for h in holdings]
                fig = go.Figure(data=go.Pie(labels=labels, values=values, marker_colors=['#00d2ff', '#00ff88', '#ffaa00'], hole=0.4))
                fig.update_layout(height=250, paper_bgcolor="#1e1e2e", font_color="#e6e6e6", title="盈亏分布")
                st.plotly_chart(fig, use_container_width=True)
                ...
                fig.update_layout(height=280, paper_bgcolor="#1e1e2e", plot_bgcolor="#1a1d24", font_color="#e6e6e6", margin=dict(l=0, r=0, t=20, b=0), xaxis_title="月份", yaxis_title="收益率(%)")
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**各月表现**")
...
if st.session_state.page == "首页":
    ...
    row1 = st.columns(4)
    with row1[0]:
        st.markdown(f"""<div class="market-card"><span>EUR/USD</span><br><span class="price">{eurusd:.5f}</span></div>""", unsafe_allow_html=True)
    with row1[1]:
        st.markdown(f"""<div class="market-card"><span>BTC/USD</span><br><span class="price">${btc:,.0f}</span></div>""", unsafe_allow_html=True)
    with row1[2]:
        st.markdown(f"""<div class="market-card"><span>黄金</span><br><span class="price">${gold:.0f}</span></div>""", unsafe_allow_html=True)
    with row1[3]:
        st.markdown(f"""<div class="market-card"><span>标普500</span><br><span class="price">4500</span></div>""", unsafe_allow_html=True)
    ...
    if st.button("执行AI分析"):
        ...
        st.info("置信度: 87% | 止盈: +2.3% | 止损: -1.2%")

elif st.session_state.page == "持仓管理":
    st.markdown("### 💼 持仓")
    st.dataframe(pd.DataFrame(get_holdings_data()), use_container_width=True)
