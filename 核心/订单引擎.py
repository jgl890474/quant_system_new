...
class 订单引擎:
    ...
    def 获取总资产(self):
        """总资产 = 初始资金 + 已实现盈亏 + 浮动盈亏"""
        浮动盈亏 = self.获取浮动盈亏()
        return self.初始资金 + self.已实现盈亏 + 浮动盈亏
    
    def 获取总盈亏(self):
        """总盈亏 = 浮动盈亏 + 已实现盈亏 = 总资产 - 初始资金"""
    ...
    def 买入(self, 品种, 价格, 数量=1000):
        ...
        if '风控引擎' in st.session_state:
            ...
            st.session_state['成功消息'] = f"✅ 买入 {品种} {数量}股 @ {价格:.4f} | 止损: ¥{止损止盈价位['止损价']:.2f} | 止盈: ¥{止损止盈价位['止盈价']:.2f}"
        else:
    ...
    def 卖出(self, 品种, 价格, 数量=1000):
        ...
        if 品种 in self.持仓 and self.持仓[品种].数量 >= 数量:
            ...
            st.session_state['成功消息'] = f"✅ 卖出 {品种} {数量}股 @ {价格:.4f}, 盈亏: ¥{盈亏:+.2f}"
            st.rerun()
        else:
