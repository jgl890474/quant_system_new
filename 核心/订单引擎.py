...
class 订单引擎:
    ...
    def 买入(self, 品种, 价格, 数量=100, 策略名称=""):
        # ========== 调试信息：打印入参 ==========
        print(f"🔍 [买入调试] 品种={品种}, 价格={价格}, 数量={数量}, 策略={策略名称}")
        ...
        需要资金 = 价格 * 数量
        print(f"🔍 [买入调试] 需要资金={需要资金}")
        
        可用资金 = self.获取可用资金()
        print(f"🔍 [买入调试] 可用资金={可用资金}")
        ...
        # 执行买入前记录原持仓
        if 品种 in self.持仓:
            print(f"🔍 [买入调试] 原持仓: {品种}, 数量={self.持仓[品种].数量}, 成本={self.持仓[品种].平均成本}")
        ...
        if 品种 in self.持仓:
            ...
            pos.平均成本 = 总成本 / 总数量
            pos.当前价格 = 价格
            print(f"🔍 [买入调试] 更新持仓: 新数量={总数量}, 新成本={pos.平均成本:.4f}")
        else:
            new_pos = 持仓数据(品种, 数量, 价格)
            new_pos.当前价格 = 价格
            self.持仓[品种] = new_pos
            print(f"🔍 [买入调试] 新增持仓: 数量={数量}, 成本={价格:.4f}")

        self.已实现盈亏 -= 需要资金
        print(f"🔍 [买入调试] 扣减后已实现盈亏={self.已实现盈亏}")
        ...
        try:
            ...
            print(f"🔍 [买入调试] 交易记录已保存到数据库")
        except Exception as e:
            print(f"🔍 [买入调试] 保存交易记录失败: {e}")

        self._保存持仓到数据库()
        print(f"🔍 [买入调试] 持仓已保存到数据库")
    ...
    def 卖出(self, 品种, 价格, 数量=100, 策略名称=""):
        # ========== 调试信息：打印入参 ==========
        print(f"🔍 [卖出调试] 品种={品种}, 价格={价格}, 数量={数量}, 策略={策略名称}")
        ...
        pos = self.持仓[品种]
        print(f"🔍 [卖出调试] 原持仓: 数量={pos.数量}, 成本={pos.平均成本}")
        
        盈亏 = (价格 - pos.平均成本) * 数量
        盈亏率 = (盈亏 / (pos.平均成本 * 数量)) * 100 if pos.平均成本 > 0 else 0
        print(f"🔍 [卖出调试] 盈亏={盈亏}, 盈亏率={盈亏率:.2f}%")

        pos.数量 -= 数量
        self.已实现盈亏 += 价格 * 数量
        pos.已实现盈亏 += 盈亏
        print(f"🔍 [卖出调试] 更新后: 持仓数量={pos.数量}, 已实现盈亏={self.已实现盈亏}")
