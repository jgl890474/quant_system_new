...
class 订单引擎:
    ...
    def 卖出(self, 品种, 价格, 数量=1):
        ...
        # 检查持仓是否足够
        if self.持仓[品种].数量 < 实际数量:
            st.session_state['错误消息'] = f"❌ 卖出失败: {品种} 持仓不足 (持有: {self.持仓[品种].数量}, 请求卖出: {实际数量})"
            st.rerun()
            return
        ...
        st.session_state['成功消息'] = f"✅ 卖出 {品种} {实际数量}单位 @ {价格:.4f}, 盈亏: ¥{盈亏:+.2f}"
        st.rerun()
