...
class 订单引擎:
    ...
    def _获取单位乘数(self, 品种):
        """根据品种类型返回单位乘数"""
        乘数映射 = {
            "000001.SS": 100,     # A股：1手=100股
            "EURUSD": 10000,      # 外汇：1手=10000单位
            "BTC-USD": 1,         # 加密货币：1个
            "GC=F": 1,            # 黄金期货：1盎司
            "AAPL": 1,            # 美股：1股
        }
    ...
    def 买入(self, 品种, 价格, 数量=1):
        ...
        if 品种 in self.持仓:
            ...
            pos.平均成本 = 总成本 / 总数量
            pos.当前价格 = 价格
            print(f"📈 更新持仓: {品种} {原数量} -> {总数量}, 成本 {原成本:.4f} -> {pos.平均成本:.4f}")
        else:
            new_pos = 持仓数据(品种, 实际数量, 价格)
            new_pos.当前价格 = 价格
            self.持仓[品种] = new_pos
            print(f"📈 新增持仓: {品种} 数量={实际数量}, 成本={价格:.4f}")
        
        # 扣减可用资金
        self.已实现盈亏 -= 需要资金
        print(f"💰 扣减资金: {需要资金:.2f}, 剩余可用: {self.获取可用资金():.2f}")
        ...
        self._保存持仓到数据库()
        print(f"💾 保存持仓到数据库: {self.持仓}")
        
        # 设置止损止盈
        if '风控引擎' in st.session_state:
            风控 = st.session_state.风控引擎
            止损止盈价位 = 风控.设置止损止盈(品种, 价格)
            st.session_state['成功消息'] = f"✅ 买入 {品种} {数量}手 ({实际数量}股/单位) @ {价格:.4f} | 止损: ¥{止损止盈价位['止损价']:.2f} | 止盈: ¥{止损止盈价位['止盈价']:.2f}"
        else:
            st.session_state['成功消息'] = f"✅ 买入 {品种} {数量}手 ({实际数量}股/单位) @ {价格:.4f}"
    ...
    def 卖出(self, 品种, 价格, 数量=1):
        ...
        if 品种 in self.持仓 and self.持仓[品种].数量 >= 实际数量:
            ...
            st.session_state['成功消息'] = f"✅ 卖出 {品种} {数量}手 ({实际数量}股/单位) @ {价格:.4f}, 盈亏: ¥{盈亏:+.2f}"
            st.rerun()
        else:
