...
class 风控引擎:
    ...
    def __init__(self, 配置=None):
        ...
        # 风控参数 - 修改为0.1%
        self.止损比例 = self.配置.get("止损比例", 0.001)      # 0.1%
        self.止盈比例 = self.配置.get("止盈比例", 0.001)      # 0.1%
        self.移动止损开关 = self.配置.get("移动止损开关", True)
        self.移动止损回撤 = self.配置.get("移动止损回撤", 0.002)  # 0.2%回撤触发
...
# 创建风控引擎实例的便捷函数
def 创建风控引擎(止损=0.001, 止盈=0.001, 移动止损=True, 移动止损回撤=0.002, 
