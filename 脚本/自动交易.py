...
class 自动交易机器人:
    ...
    def _加载策略配置(self):
        ...
        # 从策略加载器获取所有策略
        实际策略列表 = []
        if self.策略加载器 is not None and hasattr(self.策略加载器, '获取策略'):
            try:
        ...
        for s in 实际策略列表:
            ...
            self.策略配置列表.append({
                ...
                "启用": True,  # 默认启用
                "品种": [品种],
                "执行间隔": 10,  # 测试用10秒，正式可改回60
                "资金分配": 0.25,
                "最大持仓": 3,
                "参数": {}
            })
        
        print(f"✅ 自动交易加载 {len(self.策略配置列表)} 个策略")
        
        # 打印所有加载的策略名称
        for c in self.策略配置列表:
            print(f"   - {c['名称']} ({c['类别']})")
        
        # 如果没有从策略加载器获取到策略，使用默认配置
        if not self.策略配置列表:
            print("⚠️ 未从策略加载器获取到策略，使用默认配置")
            默认策略配置 = [
                {"名称": "随时买入", "类别": "加密货币", "启用": True, "品种": ["BTC-USD"], "执行间隔": 10, "资金分配": 0.25, "最大持仓": 3, "参数": {}},
                {"名称": "加密双均线1", "类别": "加密货币", "启用": True, "品种": ["BTC-USD"], "执行间隔": 60, "资金分配": 0.25, "最大持仓": 3, "参数": {}},
                {"名称": "加密风控策略", "类别": "加密货币", "启用": True, "品种": ["BTC-USD"], "执行间隔": 60, "资金分配": 0.25, "最大持仓": 3, "参数": {}},
            ]
            self.策略配置列表 = 默认策略配置
            print(f"✅ 使用默认配置，加载 {len(self.策略配置列表)} 个策略")
