...
# ========== Tab导航 ==========
首页tab, 策略中心tab, AI交易tab, 持仓管理tab, 资金曲线tab, 回测tab, 交易记录tab = st.tabs(
    ["🏠 首页", "📊 策略中心", "🤖 AI交易", "💼 持仓管理", "💰 资金曲线", "📈 回测", "📋 交易记录"]
)
...
with AI交易tab:
                ...
                if st.session_state.ai_auto_trade:
                    if 信号['信号'] == "买入" and 信号['建议数量'] > 0:
                        ...
                        if 品种 in 引擎.持仓:
                            ...
                            if 结果.get("success"):
                                st.success(f"🤖 AI自动卖出 {品种} {信号['建议数量']} 个")
                                st.session_state.ai_signal = None
                                st.rerun()
                            else:
    ...
    if st.session_state.get('ai_auto_trade', False):
        ...
        st.info("💡 开启「AI自动交易」后，系统将根据AI信号自动执行买卖")
