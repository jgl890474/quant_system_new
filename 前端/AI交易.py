...
def 获取品种列表(市场):
    ...
    品种映射 = {
        ...
        "外汇": [
            {"代码": "EURUSD=X", "名称": "欧元/美元", "波动率": 0.01, "流动性": 100},
            {"代码": "GBPUSD=X", "名称": "英镑/美元", "波动率": 0.02, "流动性": 95},
            {"代码": "USDJPY=X", "名称": "美元/日元", "波动率": 0.02, "流动性": 98},
        ],
        "期货": [
...
def 计算综合评分(品种, 策略类型, 市场, 实时价格):
    ...
    elif "风控" in 策略类型 or "加密风控" in 策略类型:
        # 风控策略：波动率评分
        if 品种["波动率"] < 0.05:
            评分 += 20  # 低波动更安全
        elif 品种["波动率"] < 0.08:
    ...
    # 随机微调（模拟市场不确定性，范围-5到+5）
    评分 += random.randint(-3, 8)
...
def 获取AI推荐(策略类型, 市场, 引擎):
    ...
    for 品种 in 品种列表:
        ...
        评分 = 计算综合评分(品种, 策略类型, 市场, 实时价格)
        
        推荐列表.append({
            "代码": 代码,
            "名称": 品种["名称"],
            "价格": 实时价格 if 实时价格 else 0,
            "评分": 评分,
            "市场": 市场,
            "理由": f"{策略类型}策略评分 {评分}分",
            "建议数量": 100,
...
def 显示(引擎, 策略加载器, AI引擎=None):
    ...
    if hasattr(st.session_state, 'ai推荐列表') and st.session_state.ai推荐列表:
        ...
        for idx, item in enumerate(st.session_state.ai推荐列表):
            ...
            with st.container():
                ...
                with col3:
                    计算数量 = int(建议数量) if 选中市场 != "加密货币" else 建议数量
                    st.caption(f"建议: {计算数量}{数量单位}")
                ...
                with col5:
                    if st.button(f"买入", key=f"ai_buy_{idx}_{code}"):
                        if price > 0:
                            # 计算买入数量
                            if 选中市场 == "A股":
                                计算数量 = max(100, int(可用资金 * 0.1 / price / 100) * 100)
                            elif 选中市场 == "加密货币":
                                计算数量 = round(可用资金 * 0.1 / price, 4)
                            else:
                                计算数量 = max(1, int(可用资金 * 0.1 / price))
    ...
    if hasattr(引擎, '持仓') and 引擎.持仓:
        ...
        for sym, pos in 引擎.持仓.items():
            ...
            持仓数据.append({
                "品种": sym,
                "数量": f"{int(数量)}" if sym not in ["ETH-USD", "BTC-USD", "SOL-USD", "BNB-USD"] else f"{数量:.4f}",
                "成本": round(平均成本, 2),
                "现价": round(当前价格, 2),
                "浮动盈亏": round(浮动盈亏, 2)
            })
