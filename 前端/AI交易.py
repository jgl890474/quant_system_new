...
def 显示(引擎, 策略加载器=None, AI引擎=None):
    ...
    try:
        ...
        print(f"📊 获取到 {len(所有策略)} 个策略")
        for s in 所有策略:
            print(f"   - {s.get('名称')} ({s.get('类别')})")
