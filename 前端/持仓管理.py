                ...
                with st.spinner(f"正在加载 {选中品种} 的{周期选项}数据..."):
                    try:
                        ...
                        if df_kline is not None and not df_kline.empty:
                            ...
                            if 买入标注 or 卖出标注:
                                st.markdown("---")
                                st.markdown("#### 🤖 AI交易信号记录")
                                
                                # 显示最近的买卖记录
                                信号记录 = []
                                for b in 买入标注[-5:]:
                                    信号记录.append(f"🟢 买入 @ {b.get('价格', 0):.2f} ({b.get('日期', '未知')}) - {b.get('策略', '手动')}")
                                for s in 卖出标注[-5:]:
                                    信号记录.append(f"🔴 卖出 @ {s.get('价格', 0):.2f} ({s.get('日期', '未知')}) - {s.get('策略', '手动')}")
                                
                                if 信号记录:
                                    with st.expander("📋 最近交易信号", expanded=True):
                                        for 记录 in 信号记录[-10:]:
                                            st.write(记录)
                            ...
                            if not df_kline.empty:
                                ...
                                if 'BB_Upper' in df_indicators.columns and 'BB_Lower' in df_indicators.columns:
                                    bb_upper = df_indicators['BB_Upper'].iloc[-1]
                                    bb_lower = df_indicators['BB_Lower'].iloc[-1]
                                    bb_position = (最新收盘 - bb_lower) / (bb_upper - bb_lower) * 100 if (bb_upper - bb_lower) > 0 else 50
                                    st.caption(f"📊 布林带位置: {bb_position:.1f}% | 上轨: ¥{bb_upper:.2f} | 下轨: ¥{bb_lower:.2f}")
                                
                                # MACD信号
                                if 'MACD' in df_indicators.columns and 'MACD_Signal' in df_indicators.columns:
                                    macd = df_indicators['MACD'].iloc[-1]
                                    macd_signal = df_indicators['MACD_Signal'].iloc[-1]
                                    macd_hist = df_indicators['MACD_Histogram'].iloc[-1]
                                    macd_status = "金叉" if macd > macd_signal else "死叉"
                                    st.caption(f"📈 MACD: {macd_status} | 柱状图: {macd_hist:.4f}")
                        else:
...
def 获取增强交易标注(品种, df_kline, 引擎, AI引擎):
    ...
    # 2. 尝试从AI引擎获取历史信号
    if AI引擎 and hasattr(AI引擎, '获取信号记录'):
        try:
            ai_signals = AI引擎.获取信号记录(品种)
            for signal in ai_signals:
                标注 = {
                    "日期": signal.get('日期'),
                    "价格": signal.get('价格', 0),
                    "策略": f"AI_{signal.get('策略', '未知')}"
                }
                if signal.get('动作') == '买入':
                    买入标注.append(标注)
                elif signal.get('动作') == '卖出':
                    卖出标注.append(标注)
        except:
            pass
    
    # 去重并排序
    买入标注 = sorted(买入标注, key=lambda x: x['日期'], reverse=True)[-10:]
    卖出标注 = sorted(卖出标注, key=lambda x: x['日期'], reverse=True)[-10:]
