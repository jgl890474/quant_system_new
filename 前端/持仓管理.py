        ...
        if 数据:
            ...
            if 选中品种:
                ...
                with st.spinner(f"正在加载 {选中品种} 的{周期选项}数据..."):
                    try:
                        # 获取K线数据
                        df_kline = 获取K线数据(选中品种, 周期选项, 数据长度)
                        
                        if df_kline is not None and not df_kline.empty and len(df_kline) > 0:
                            # 计算技术指标
                            ...
                            # 安全获取最新价格
                            最新价 = df_kline['收盘'].iloc[-1] if not df_kline['收盘'].isna().iloc[-1] else 0
                            最高价 = df_kline['最高'].max() if not df_kline['最高'].isna().all() else 0
                            最低价 = df_kline['最低'].min() if not df_kline['最低'].isna().all() else 0
                            
                            # 计算涨跌幅
                            if len(df_kline) > 1:
                                前收盘 = df_kline['收盘'].iloc[-2]
                                现收盘 = df_kline['收盘'].iloc[-1]
                                if 前收盘 and 前收盘 > 0 and not pd.isna(前收盘) and not pd.isna(现收盘):
                                    涨跌幅 = (现收盘 - 前收盘) / 前收盘 * 100
                                else:
                                    涨跌幅 = 0
                            else:
                            ...
                            col1.metric("最新价", f"¥{最新价:.2f}" if 最新价 > 0 else "获取中", delta=f"{涨跌幅:.2f}%" if 涨跌幅 != 0 else None)
                            col2.metric("最高价", f"¥{最高价:.2f}" if 最高价 > 0 else "获取中")
                            col3.metric("最低价", f"¥{最低价:.2f}" if 最低价 > 0 else "获取中")
                            
                            # MA5
                            if 'MA5' in df_indicators.columns and len(df_indicators) > 0:
                                ma5_val = df_indicators['MA5'].iloc[-1]
                                col4.metric("MA5", f"¥{ma5_val:.2f}" if not pd.isna(ma5_val) and ma5_val > 0 else "计算中")
                            else:
                                col4.metric("MA5", "计算中")
                            
                            # MA20
                            if 'MA20' in df_indicators.columns and len(df_indicators) > 0:
                                ma20_val = df_indicators['MA20'].iloc[-1]
                                col5.metric("MA20", f"¥{ma20_val:.2f}" if not pd.isna(ma20_val) and ma20_val > 0 else "计算中")
                            else:
                                col5.metric("MA20", "计算中")
                        else:
    ...
    try:
        ...
        if code_str.isdigit() and len(code_str) == 6:
            if code_str.startswith('6'):
                ticker = f"{code_str}.SS"
            else:
                ticker = f"{code_str}.SZ"
        else:
            ticker = 代码
...
def 生成模拟K线数据(代码, 周期, 长度):
    ...
    base_price = 价格基准.get(str(代码), 100)
