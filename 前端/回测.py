...
def 显示(引擎=None):
    ...
    # 如果没有持仓，显示提示
    if not 持仓品种列表:
        st.warning("⚠️ 当前没有持仓，请先在AI交易中买入品种后再进行回测")
    ...
    with st.sidebar:
        ...
        # 从持仓中选择品种
        品种选项 = {f"{s['名称']}": s["代码"] for s in 持仓品种列表}
        品种显示名称 = list(品种选项.keys())
        选中品种显示 = st.selectbox("选择持仓品种", 品种显示名称, help="从当前持仓中选择要回测的品种")
        品种代码 = 品种选项.get(选中品种显示, 持仓品种列表[0]["代码"])
        
        # 显示品种类型
        选中品种类型 = next((s["类型"] for s in 持仓品种列表 if s["代码"] == 品种代码), "未知")
        st.caption(f"📌 品种类型: {选中品种类型} | 当前持仓数量: {len(持仓品种列表)}")
        ...
        手续费率 = st.number_input("手续费率", value=0.001, step=0.0005, format="%.4f", help="加密货币建议0.1%，A股万分之三")
    ...
    # 显示当前持仓列表
    with st.expander("📋 当前持仓列表", expanded=False):
        持仓显示数据 = []
        ...
        st.dataframe(pd.DataFrame(持仓显示数据), use_container_width=True, hide_index=True)
    ...
    if not 运行按钮:
        ...
        with st.expander("📈 示例：动态净值曲线 + 动态回撤曲线（点击展开）"):
            ...
            st.caption("注：此为演示数据，实际回测请选择持仓品种并点击「开始回测」")
        return
    ...
    with st.spinner(f"正在获取 {品种代码} 的历史数据并执行回测..."):
        try:
            ...
            显示回测结果(回测结果, 品种代码, 短期均线, 长期均线, 止盈百分比, 止损百分比, 策略类型, 选中品种类型)
...
def 显示回测结果(回测结果, 品种代码, 短期均线, 长期均线, 止盈百分比, 止损百分比, 策略类型, 品种类型):
    """显示回测结果"""
    st.markdown("---")
    st.subheader("📊 回测指标")
