...
def 显示(引擎=None):
    ...
    if 引擎 and hasattr(引擎, '持仓') and 引擎.持仓:
        for 代码 in 引擎.持仓.keys():
            if 代码 and not 代码.startswith(('AAPL', 'GOOG', 'MSFT')):  # 过滤美股
    ...
    with st.sidebar:
        ...
        股票显示名称默认 = 股票显示名称[0] if 股票显示名称 else ""
        
        选中股票显示 = st.selectbox(
            "选择股票", 
            股票显示名称,
            help="从持仓、策略库或常用A股中选择"
        )
    ...
    if not 运行按钮:
        ...
        with st.expander("📈 示例：动态净值曲线 + 动态回撤曲线（点击展开）"):
            dates = pd.date_range("2023-01-01", periods=200, freq='ME')
            # 模拟净值数据
            np.random.seed(42)
...
def 计算回撤(回测结果):
    """计算动态回撤"""
    净值 = np.array(回测结果['每日净值'])
    
    # 计算历史高点
    历史高点 = np.maximum.accumulate(净值)
    # 计算回撤
    回撤 = (净值 - 历史高点) / 历史高点 * 100
    # 最大回撤
    最大回撤 = np.min(回撤)
...
def 绘制动态净值曲线(df, 回测结果, 股票代码):
    ...
    if 买入记录:
        ...
        买入价格 = [回测结果['每日净值'][回测结果['数据']['日期'].tolist().index(d)] for d in 买入日期 if d in 回测结果['数据']['日期'].tolist()]
        if 买入价格:
            fig.add_trace(
                go.Scatter(
                    x=买入日期,
    ...
    if 卖出记录:
        ...
        卖出价格 = [回测结果['每日净值'][回测结果['数据']['日期'].tolist().index(d)] for d in 卖出日期 if d in 回测结果['数据']['日期'].tolist()]
        if 卖出价格:
            fig.add_trace(
                go.Scatter(
                    x=卖出日期,
...
def 显示回测结果(回测结果, 股票代码, 短期均线, 长期均线, 止盈百分比, 止损百分比):
    ...
    st.info(f"📉 最大回撤为 {最大回撤:.2f}%，风险控制{'较好' if abs(最大回撤) < 20 else '需要关注'}")
