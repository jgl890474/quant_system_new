...
def 显示(引擎=None):
    ...
    try:
        ...
        for 股票 in 策略股票:
            if isinstance(股票, dict):
                ...
                代码 = str(股票)
                名称 = 代码
            if 代码 and 代码 not in [s["代码"] for s in 可选股票]:
    ...
    # 主界面提示
    st.markdown("""
    > 💡 **使用说明**：从左侧下拉框选择股票（支持持仓股、策略库股、常用股），设置参数后点击「开始回测」。
    > 
    ...
    # ==================== 执行回测 ====================
    with st.spinner(f"正在获取 {股票代码} 的历史数据并执行回测，请稍候..."):
        try:
            # 获取股票历史数据
...
def 绘制动态净值曲线(df, 回测结果, 股票代码):
    ...
    if not 买入点.empty:
        idx = [min(i, len(回测结果['每日资产'])-1) for i in 买入点.index if i < len(回测结果['每日资产'])]
        if idx:
            fig.add_trace(
                go.Scatter(
                    x=买入点['日期'],
                    y=[回测结果['每日资产'][i] for i in idx],
                    mode='markers',
                    name='买入信号',
    ...
    if not 卖出点.empty:
        idx = [min(i, len(回测结果['每日资产'])-1) for i in 卖出点.index if i < len(回测结果['每日资产'])]
        if idx:
            fig.add_trace(
                go.Scatter(
                    x=卖出点['日期'],
