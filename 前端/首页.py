...
def 显示(引擎, 策略加载器=None, AI引擎=None):
    ...
    # ==================== 调试信息（验证用） ====================
    with st.expander("📊 数据验证（调试用）"):
        st.write(f"初始资金: ¥{初始资金:,.0f}")
        st.write(f"可用资金: ¥{可用资金:,.0f}")
        st.write(f"总成本: ¥{总成本:,.0f}")
        st.write(f"总市值（现价）: ¥{总市值:,.0f}")
        st.write(f"总浮动盈亏: ¥{总浮动盈亏:+,.0f}")
        st.write(f"---")
        st.write(f"持仓市值 = 总市值 = ¥{总市值:,.0f}")
        st.write(f"总资产 = 可用资金 + 持仓市值 = ¥{可用资金 + 总市值:,.0f}")
        st.write(f"总盈亏 = 总资产 - 初始资金 = ¥{(可用资金 + 总市值) - 初始资金:+,.0f}")
        ...
        if 引擎.持仓:
            ...
            for 品种, pos in 引擎.持仓.items():
                成本价 = pos.平均成本
                现价 = getattr(pos, '当前价格', 成本价)
                st.write(f"  {品种}: 成本 ${成本价:.2f} → 现价 ${现价:.2f} (差价: ${现价-成本价:+.2f})")
